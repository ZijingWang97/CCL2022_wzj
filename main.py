...
def main():
    ...
    if config.do_test:
        ...
        # 输出预测结果
        pred_records = framework.evaluate_without_oracle(config, model, dev_loader, config.device, config.seq_length, config.id_type, config.id_args, config.ty_args_id)
